...
describe('EmployeeService', () => {
  ...
  describe('#Error Handler', () => {
    ...
    it('should throw network error with status 0 or -1 when error is error event object', () => {
      const service = TestBed.get(EmployeeService);
      const mockErrorMsg = 'Unreachable : Server connection failed';
      const mockNetWorkMsg = 'Network Failure';
      let mockError = Object.assign(new ErrorEvent('error', {message: mockNetWorkMsg}), {status: 0});
      service.errorHandle({error: mockError}).subscribe(
        (response) => {
      ...
      mockError = Object.assign(new ErrorEvent('error', {message: mockNetWorkMsg}), {status: 1});
      service.errorHandle({error: mockError}).subscribe(
        (response) => {
          console.log(response);
        },
        (error) => {
          expect(error).toEqual(mockNetWorkMsg);
        }
      )
    });

    it('should through api error with status 0 or -1', () => {
      const service = TestBed.get(EmployeeService);
      const mockErrorMsg = 'Unreachable : Server connection failed';
      const mockBadReq = 'Error : Bad Request';
      let mockError = {status: 0, error: { message: ''}};
      service.errorHandle(mockError).subscribe(
        (response) => {
          console.log(response);
        },
        (error) => {
      ...
      mockError = {status: 500, error: { message: ''}};
      service.errorHandle(mockError).subscribe(
        (response) => {
          console.log(response);
        },
        (error) => {
          expect(error).toEqual(mockBadReq);
        }
      )
    });

    it('should through api error with status 403 unauthorized.', () => {
      const service = TestBed.get(EmployeeService);
      const mockUnAuthMsg = 'Un-Authorized user...';
      const mockError = {status: 403, error: { message: mockUnAuthMsg}};
      service.errorHandle(mockError).subscribe(
        (response) => {
          console.log(response);
        },
        (error) => {
          expect(error).toEqual('Error : ' + mockUnAuthMsg);
        }
      )
    });
  });
